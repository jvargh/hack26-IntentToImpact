...
Publish `DEMO-CASE-CLAIMS-V3` and its versioned contracts/fixtures before future NSP
...
```text
apps\experience\                 shared prototype and production UI
apps\control-plane\              loopback API, local orchestrator entry and workers
contracts\                      schemas, generated type definitions, capability registry
engines\                        requirements, model, decision, review, generation, drift
adapters\                       Foundry, local workspace/commands, Azure
reporting\                      graph, attention, away and outcome projections
fixtures\experience\             versioned projection scenarios
design\                         journey map, tokens, component inventory, UX decision records
tools\                          PowerShell operator harness and structured command client
tests\                          unit, contract, integration, browser, failure and live-proof
...
evidence origin/checksums, decision, affected contracts/fixtures/plan IDs and unresolved
...
```powershell
.\tools\run-hero.ps1 -RunId demo-001 -Mode Live
.\tools\show-local-diff.ps1 -CaseId CASE-001 -ChangeSetId CHANGE-001
.\tools\apply-approved-change.ps1 -CaseId CASE-001 -ChangeSetId CHANGE-001 -ExpectedRevision 18
.\tools\verify-restoration.ps1 -CaseId CASE-001 -FindingId DRIFT-001
```
...
| 3: Test-focused | Loading, reconnect, retry, recovery edges and secondary viewport permutations | Functional and covered by fixtures/tests; do not spend equal visual polish effort |
...
| T-LIVE | Mandatory proof rows in section 12 | Live origin, current timestamps, exact hashes and legitimate scope; fixtures/replays cannot pass |
...
Production UI components can be built against fixtures while engines/integrations
...
decomposed later. The next table supplies explicit scope/contracts/test/promotion details
...
M0.5 contracts/implementations from later completion dependencies; do not make early
...
| UX-04 | Accessibility and responsive design review | D | MOCK-02, SPK-06 | P0 | Y | M | Required interactions accessible; M9 | Keyboard/zoom/mobile | Local browser checks | Corrected fixtures/layout | UX04/05/06 |
| MOCK-01 | Schema-backed projection fixtures/transport | A | FND-01, UX-01 | P0 | Y | H | FX-00..19 validate; M2 | Realistic states | No real side effects | Full state catalog | UX01-05 |
