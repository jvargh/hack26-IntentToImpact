BeforeAll {
    ...
    . $stopScript
    $script:workspace = 'C:\repo\intent-to-impact-studio'
    ...
    $script:pythonCommand = '"C:\repo\.intent-to-impact\studio\.venv\Scripts\python.exe" -B -m studio.serve'
    $script:viteCommand = '"node" "C:\repo\intent-to-impact-studio\apps\experience\node_modules\.bin\\..\vite\bin\vite.js" --host 127.0.0.1 --port 5173 --strictPort'
}
...
Describe 'Workspace server recognition' {
    ...
    It 'does not recognize unrelated processes, similarly named roots or missing metadata' {
        foreach ($command in @(
            ...
            '"C:\repo\.intent-to-impact\studio\.venv\Scripts\python.exe" unrelated.py',
            '"node" "C:\repo\intent-to-impact-studio-other\apps\experience\node_modules\vite\bin\vite.js"',
            '"node" unrelated.js',
...
Describe 'Listener query failures' {
    ...
    Describe 'Direct script invocation' {
        BeforeEach {
            ...
            $script:repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $stopScript))
