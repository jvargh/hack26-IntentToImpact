...
| --- | --- | --- | --- |
| C-DOM | `intent-to-impact-studio\contracts\domain\`, generated type outputs | A / E | Schema/test artifacts only |
| C-EXP | `intent-to-impact-studio\contracts\experience\`, `intent-to-impact-studio\contracts\events\`, capability registry | A / C/E | Projection/action contract outputs |
| STORE | `intent-to-impact-studio\apps\control-plane\state\`, `intent-to-impact-studio\engines\case\` | A / E | `runs\<id>\cases\<id>\case.json`, derived events only |
| POLICY | `intent-to-impact-studio\apps\control-plane\policy\` | A / B/E | Decision/denial events, no cloud grants |
| WORK | `intent-to-impact-studio\apps\control-plane\work\` | A / E | Bounded work/event/idempotency records inside case state |
| API | `intent-to-impact-studio\apps\control-plane\api\` | A / C/E | Authorized projections and commands; no arbitrary paths |
| REQ | `intent-to-impact-studio\engines\requirements\` | A / E | Confirmed requirements/promises via STORE |
| DESIGN | `intent-to-impact-studio\engines\architecture-model\`, `intent-to-impact-studio\engines\review-gates\`, `intent-to-impact-studio\engines\decisions\` | A / B/E | Logical sections have separate owners through STORE |
| EVAL | `intent-to-impact-studio\engines\drift\`, promise verifier registry | A / B/E | Findings/evaluations and evidence refs |
| REPORT | `intent-to-impact-studio\reporting\` | A / C/E | Immutable graph/receipt exports and derived attention/away |
| FOUNDRY | `adapters\foundry\`, `intent-to-impact-studio\apps\control-plane\orchestrator\`, `agents\` | B / A/E | Real invocation receipts; no agent approval tools |
| LOCAL | `adapters\local\`, `intent-to-impact-studio\engines\generation\`, approved `templates\bicep\` | B, materialization coordinator A / E | Immutable candidates/materialized files, command receipts |
| AZURE | `adapters\azure\` | B / A/E | Read-only normalized provider evidence |
| HARNESS | `intent-to-impact-studio\tools\` | E / A/B | Explicit owned run setup, operator receipts and scoped commands |
| UI | `intent-to-impact-studio\apps\experience\src\` | C / D/E | Browser read/action events; no local filesystem writes |
| UX | `intent-to-impact-studio\design\`, shared UI token/component contracts | D / C/A | Journey, tokens, checkpoint/UXD records |
| FIX | `intent-to-impact-studio\fixtures\experience\`, `intent-to-impact-studio\fixtures\scenarios\`, fixture transport | A owns shape; D authors reviewed content / E/C | Mock responses never stored as live evidence; scenario text can be a confirmed input to a fresh live run |
| TEST | `intent-to-impact-studio\tests\` matching subject subtree | E; implementation owner adds unit tests / relevant owner | Test outputs and proof references |
...
One versioned scenario artifact under `intent-to-impact-studio\fixtures\scenarios\` is the stable shared input
