...
OWNED = WORKSPACE / ".intent-to-impact" / "spikes" / "SPK-03-01"
APPROVAL_ID = "APPROVAL-SANDBOX-CREATE-001"
APPROVAL_FILE = WORKSPACE / ".intent-to-impact" / "execution" / "decisions" / f"{APPROVAL_ID}.json"
...
class Run:
    def __init__(self, execute):
        for path in [WORKSPACE / ".intent-to-impact", OWNED.parent, OWNED]:
        ...
        self.receipt = {
            ...
            "approvalReference": str(APPROVAL_FILE.relative_to(WORKSPACE)),
    ...
    def redact(self, text):
        for value, label in [(str(WORKSPACE), "[workspace]"), (str(Path.home()), "[user-home]")]:
...
def main():
    ...
    print(json.dumps({"status": run.receipt["status"], "baselineStatus": run.receipt["baselineStatus"],
                      "receipt": str((run.root / "receipt.json").relative_to(WORKSPACE)),
