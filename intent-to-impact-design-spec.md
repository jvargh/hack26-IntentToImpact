...
agent or editable architecture diagram. The existing reporting/projection service
...
| Intent Continuity Graph and While You Were Away projections | Existing reporting/projection service | Derived from authorized canonical artifacts and durable events; no separate source of truth |
...
| Runtime evidence | Required live read-only observation of explicitly authorized sandbox resources through Azure Resource Graph and/or service APIs; fixtures for tests/disclosed fallback only |
...
| Read graph or away summary | Authorized derived projections via experience API; graph uses the scoped continuity endpoint | Existing reporting/projection service |
